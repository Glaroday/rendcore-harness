...
function bundledNodePath(): string {
  return join(app.getAppPath(), 'node_modules', 'node-win-x64', 'bin', 'node.exe')
}
  ...
  harnessLaunchOperation = (async () => {
    ...
    const maintenance = await runProfileStartupMaintenance({
      ...
      preparePackageStore: async () => {
        ...
        const retiredMarketGeneration = await uninstallGenerationPlugin(
          dshHome,
          'dshmarket',
        ...
        if (upgradedMarket || retiredMarketGeneration) {
          ...
          if (!result.ok) {
            throw new Error(
              `dshmarket compatibility repair failed: ${result.detail ?? 'unknown package error'}`
            )
          }
          await markProfileInstallComplete(dshHome)
          await prepareGenerationsForLaunch(dshHome, (line) => runtime.note(line))
          runtime.note(
            `[desktop] installed the compatible dshmarket shared-tree baseline ^${VERIFIED_MARKET_BASELINE}`
          )
        }
      },
      enforcePendingPluginRemovals: () =>

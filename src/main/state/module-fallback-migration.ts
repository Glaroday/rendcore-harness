...
export async function clearLegacyModuleFallbackConflicts(options: {
  ...
  const candidates: Array<{
    ...
    location: 'shared fallback' | 'web profile'
  }> = []
  for (const [modulesDirectory, location] of [
    [join(options.dshHome, 'profiles', 'node_modules'), 'shared fallback'],
    [join(options.dshHome, 'profiles', 'web', 'node_modules'), 'web profile']
  ] as const) {
    let entries
    try {
      entries = await readdir(modulesDirectory, { withFileTypes: true })
    } catch (error) {
    ...
    for (const entry of entries) {
      ...
      if (!entry.name.startsWith('@')) {
        if (location === 'web profile' && entry.name !== 'dsh-model-fix') continue
        candidates.push({
          name: entry.name,
          path: join(modulesDirectory, entry.name),
  ...
  for (const candidate of candidates) {
    ...
    options.note?.(
      `[desktop] removed legacy module fallback conflict from ${candidate.location}: ${candidate.name}`
    )
  }
  return removed
}

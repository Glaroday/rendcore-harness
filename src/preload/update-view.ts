...
export function updateMessage(status: UpdateStatus, locale: UpdateLocale): string {
  ...
  switch (status.phase) {
    ...
    case 'available':
      return zh
        ? `发现新版本${version}，是否更新？`
        : `RendCore Harness${version} is available. Update now?`
    case 'downloading': {
      const percent = Math.round(status.percent ?? 0)
      return zh ? `正在下载更新 ${percent}%` : `Downloading update ${percent}%`
    }
    case 'downloaded':
      return zh ? `RendCore Harness${version} 已下载完成` : `RendCore Harness${version} is ready to install`
    case 'up-to-date':
      return zh ? 'RendCore Harness 已是最新版本' : 'RendCore Harness is up to date'
    case 'unsupported':

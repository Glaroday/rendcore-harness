...
RendCore Harness 默认从 GitHub Releases 检查并下载更新。安装版启动后会在后台检查，镜像只影响更新 feed，不影响 Harness、模型服务或 API 密钥。
...
删除这个文件或把 `mirrors` 改为空数组即可恢复默认设置。高级用户也可以设置环境变量 `RENDCORE_UPDATE_FEED_URL`，它会优先于文件中的地址。

...
describe('LAN mobile page', () => {
  ...
  it('renders an adaptive reconnect action for the Home Screen app', () => {
    ...
    expect(zh).toContain('请确保手机和电脑连接到同一 Wi-Fi。点击重新连接后，在电脑上的 RendCore Harness 中允许此手机。')
    ...
    expect(tunnelZh).toContain('点击重新连接，然后在电脑上的 RendCore Harness 中允许此移动设备。')
  ...
  it('uses DSH styling on both pairing surfaces', () => {
    ...
    expect(phone).toContain('Cannot reach the desktop. Start RendCore Harness and try again.')
  ...
  it('localizes both pairing surfaces from the desktop preference', () => {
    ...
    expect(phone).toContain('请在 RendCore Harness 中确认连接请求。')
    expect(phone).toContain('再次发起申请')
    expect(phone).toContain('暂时无法连接桌面端，请先启动 RendCore Harness。')
  })

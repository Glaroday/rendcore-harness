...
describe('profile repair', () => {
  ...
  it('upgrades an older dshmarket baseline to the verified version', async () => {
    ...
    const installedMarket = join(home, 'profiles', 'web', 'node_modules', 'dshmarket')
    await mkdir(installedMarket, { recursive: true })
    await writeFile(
      join(installedMarket, 'package.json'),
      JSON.stringify({ name: 'dshmarket', version: VERIFIED_MARKET_BASELINE }),
      'utf8'
    )
    // Running again after the compatible package is materialized returns false.
  ...
  it('repairs a declared market package that is missing from disk', async () => {
    const { home } = await profileHome()
    const manifestPath = join(home, 'profiles', 'web', 'package.json')
    await writeFile(
      manifestPath,
      JSON.stringify({ dependencies: { dshmarket: `^${VERIFIED_MARKET_BASELINE}` } }),
      'utf8'
    )

    await expect(ensureMinimumMarketBaseline(home)).resolves.toBe(true)
  })
})

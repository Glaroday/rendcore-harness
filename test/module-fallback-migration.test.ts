...
describe('module fallback migration', () => {
  ...
  it('removes only physical installation packages left in the shared fallback', async () => {
    ...
    const web = join(home, 'profiles', 'web', 'node_modules')
    ...
    await packageAt(web, 'dsh-model-fix')
    await packageAt(web, 'undici')
    ...
    expect(removed.sort()).toEqual(['@liustack/modsearch', 'dsh-model-fix', 'dsh-model-fix', 'undici'])
    expect(existsSync(join(shared, 'dsh-model-fix'))).toBe(false)
    expect(existsSync(join(web, 'dsh-model-fix'))).toBe(false)
    expect(existsSync(join(web, 'undici'))).toBe(true)
